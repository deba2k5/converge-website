...
const EventsSection = () => {
  ...
  const EventCard = ({ event, index }: { event: Event; index: number }) => (
    ...
    >
      <div className="grid grid-cols-4 gap-4 items-center py-4 border-b border-border/30 hover:border-primary/50 transition-colors cursor-pointer">
        <span className="text-foreground/60 font-display">{event.id}</span>
        <span className="text-foreground font-medium col-span-1 group-hover:text-primary transition-colors">{event.name}</span>
        <span className="text-foreground/80 font-display text-right">{event.date}</span>
        <span className="text-foreground/80 font-display text-right group-hover:text-primary transition-colors">{event.time}</span>
      </div>
    </motion.button>
  );
